...
NSString *ASIHTTPRequestVersion = @"v1.6.2-34 2010-06-03";
...
- (void)updateProgressIndicators
{
	...
	if (![self mainRequest]) {
		if ([self showAccurateProgress] || ([self isComplete] && ![self updatedProgress])) {
...
- (void)updateDownloadProgress
{
	...
	if (![self responseHeaders] || [self needsRedirect] || !([self contentLength] || [self isComplete])) {
		return;
	}
